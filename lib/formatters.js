...
function formatAllInventory({ items, truncated }) {
  ...
  const rows = items
    .slice(0, 40)
    .map((p) => `• *${p.sku}* — Available: \`${p.available}\` | On Hand: ${p.on_hand} | Allocated: ${p.allocated}${p.backorder > 0 ? ` | ⚠️ Backorder: ${p.backorder}` : ""}`)
    .join("\n");

  const blocks = [
    {
      type: "header",
      text: { type: "plain_text", text: `🏭 Available Inventory — ${items.length} SKUs` },
    },
    { type: "divider" },
