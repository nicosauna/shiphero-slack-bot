function formatAllBackorders(backorders) {
  ...
  if (backorders.length > 40) {
    blocks.push({
      type: "context",
      elements: [
        {
          type: "mrkdwn",
          text: `_Showing top 40 of ${backorders.length} SKUs. Use \`/backorder-sku [sku]\` to search specific ones._`,
        },
      ],
    });
  }
...
function formatAllInventory(items) {
  if (!items.length) {
    return {
      response_type: "in_channel",
      blocks: [
  ...
  if (items.length > 40) {
    blocks.push({
      ...
      elements: [
        {
          type: "mrkdwn",
          text: `_Showing 40 of ${items.length} SKUs. Use \`/inventory-sku [sku]\` for a specific one._`,
        },
      ],
    });
  }
